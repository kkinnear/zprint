...
that it will startup in less than a second.
  ...
  1. Download `zprint-filter-0.4.2` from [releases](https://github.com/kkinnear/zprint/releases/latest)
...
Here are the steps to set this version up manually:
  ...
  java -XX:+UnlockCommercialFeatures -XX:+UseAppCDS -Xshare:off \
	  ...
	  -cp zprint-filter-0.4.2 \
          zprint.main  < helloworld.clj > /dev/null
  ```
  or, for easier copying and pasting: 

  ```
  java -XX:+UnlockCommercialFeatures -XX:+UseAppCDS -Xshare:off -XX:DumpLoadedClassList=zprint.filter.classlist -cp zprint-filter-0.4.2 zprint.main < helloworld.clj > /dev/null
  ```  
  ...
  ```
  java -XX:+UnlockCommercialFeatures -XX:+UseAppCDS -Xshare:dump -XX:SharedClassListFile=zprint.filter.classlist -XX:SharedArchiveFile=`pwd`/zprint.filter.cache -cp `pwd`/zprint-filter-0.4.2 zprint.main < helloworld.clj 
  ```  
  ...
  java -XX:+UnlockCommercialFeatures -XX:+UseAppCDS -Xshare:on \
          -XX:SharedArchiveFile=`pwd`/zprint.filter.cache \
	  -cp `pwd`/zprint-filter-0.4.2 zprint.main
  ```
  or, for easier copying and pasting:

  ```
  java -XX:+UnlockCommercialFeatures -XX:+UseAppCDS -Xshare:on -XX:SharedArchiveFile=`pwd`/zprint.filter.cache -cp `pwd`/zprint-filter-0.4.2 zprint.main
  ```  
...
### bootclasspath version
  ...
  ```
  java -Xbootclasspath/a:`pwd`/zprint-filter-0.4.2 zprint.main
  ```
