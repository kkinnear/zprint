...
You can download a pre-build Linux image from zprint GitHub, look for `zprintl-0.4.14`
(or the appropriate version) in the latest zprint release.

### MacOS

You can now download a pre-build MacOS image from zprint GitHub, look for `zprintm-0.4.14`
(or the appropriate version) in the latest zprint release.

## Usage

The `zprintl-0.4.14` or `zprintm-0.4.14` It is a regular "unix-like" filter which will
run on Linux or MacOS respectively -- __without any JVM__ involved!.  You
use it like:

```
./zprintm-0.4.14 < core.clj > core.new.clj
```
...
```
./zprintm-0.4.14 '{:style :community}' < core.clj > core.new.clj
```
