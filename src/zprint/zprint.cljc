...
(defn guided-output-alt
  ...
  ; [caller options next-seq zloc next-guide cur-index guide-seq
  ;  index multi-fit? param-map mark-map previous-data out]
  [caller
   {:keys [width rightcnt],
    {:keys [wrap-after-multi? respect-nl?]} caller,
    :as options} next-seq zloc rightmost-zloc? next-guide #_cur-seq #_cur-zloc
   cur-index guide-seq guide-index index multi-fit?
  ...
  (let [guided-newline? (= next-guide :newline)
        #_#_next-seq (when guided-newline? [["\n" :none :newline]])
        ...
        ; where we are and where the next thing begin.
        regular-space (if (or previous-newline? (zero? guide-index)) 0 1)
        additional-spaces (or align-spaces spaces 0)
        beyond-cur-ind (max additional-spaces regular-space)
        actual-spaces (if previous-newline?
                        (+ beyond-cur-ind cur-ind)
                        beyond-cur-ind
                        #_(max 1 beyond-cur-ind))
        fzprint*-options (if rightmost-zloc? options (not-rightmost options))
        hang-ind (+ beyond-cur-ind cur-ind)
        try-hang? (and zloc
                       (not previous-newline?)
                       (< cur-ind width)
                       (< hang-ind width))
        hang-result (when try-hang?
                      (fzprint* (in-hang fzprint*-options) hang-ind zloc))
        hang-lines (style-lines options hang-ind hang-result)
        hang-fit? (fzfit options hang-lines)
        hang-multi? (when hang-result (> (count hang-result) 1))
        #_(and hang-lines (> (first hang-lines) 1))
        hang-linecnt (when hang-lines (first hang-lines))
        fail-fit? (and zloc
                       (or (empty? hang-result)
                           (and hang-multi? (> hang-linecnt 1))))
        ; If it didn't fit, we will flow it.  But if we flow it, we need
        ; to reset cur-ind.
        ;
        ; EXCEPT THIS ISN'T REALLY cur-ind.  It will be 0 once we do
        ; an \n.  But we need to ensure that we do prepend-nl.
        cur-ind (if fail-fit? (+ indent ind) cur-ind)
        _ (dbg-s options
                 :guide
                 "guided-output: hang-lines:" hang-lines
                 "hang-multi?" hang-multi?
                 "hang-linecnt:" hang-linecnt
                 "hang-ind:" hang-ind
                 "fail-fit?" fail-fit?)
        align-spaces (if fail-fit?
                       ; We don't have any spaces for a flow
                       (when align-ind align-ind)
                       align-spaces)
        spaces-after-indent (or spaces 0)
        prepend-nl-ind
          (or align-ind
              (+ (if (zero? guide-index) one-line-ind (+ indent ind))
                 spaces-after-indent))
        flow-result (when (and zloc
                               (or (empty? hang-result)
                                   (and hang-multi? (> hang-linecnt 1))))
                      (fzprint* fzprint*-options prepend-nl-ind zloc))
        flow-lines
          (style-lines options prepend-nl-ind #_(+ indent ind) flow-result)
        flow-fit? (fzfit options flow-lines)
        _ (dbg-s options
                 :guide
                 "guided-output: flow-lines:" flow-lines
                 "prepend-nl-ind:" prepend-nl-ind
        ...
        _ (dbg-s options
                 :guide
                 "guided-output: flow?" (not (empty? flow-result))
                 "hang?" (not (empty? hang-result)))
        next-seq (or flow-result hang-result next-seq)
        ...
        ; the start of the next-seq, which means a newline.
        indent? (= (nth (first next-seq) 2) :indent)
        ...
        [linecnt max-width lines]
          (when next-seq
            (or flow-lines hang-lines (style-lines options cur-ind next-seq)))
        next-seq-lines (style-lines options (inc cur-ind) next-seq)
        last-width (last lines)
        ...
        ; line by definition.
        len (if last-width (max 0 (- last-width cur-ind #_ind)) 0)
        ...
        fit?
          (and (not newline?)
               ...
               (or (zero? index)
                   ; hang-pairs
                   ; multi-fit? tells us whether a (possibly) multi-line
                   ; style-vec was generated at the "right" place, in which
                   ; case multi-fit? should be true, or just from fzprint-seq
                   ; which would generate a multi-line thing at
                   ; (+ indent ind) and so it could never end up on the same
                   ; line as something to the left of it because the 2nd
                   ; and subsequent lines wouldn't be in the right place.
                   (and
                     (if multi-fit? true (if multi? (= linecnt 1) true))
                     ; TODO: If I understand cur-ind, this needs a space
                     ; We know this is not the first thing, (presumably
                     ; because of the (zero? index) checks above)
                     ; so we need to add a space before it.
                     ;
                     ; Ah.  Yes, normally this needs a space, but
                     ; that is because style-lines didn't have a
                     ; space in it.  So there is a difference between
                     ; the first and second lines here -- the second
                     ; lines are really this long, and a space messes
                     ; things up, and the first need the space.
                     ;
                     ; We need to put the space in the style-lines,
                     ; and take it out here.
                     ; And test that.
                     ;
                     ; Note that len is the last width, not the longest
                     ; width if we doing multi-fit? true. But at present
                     ; the fit is handled by fzprint-pairs before we
                     ; get here, so this is superflous for multi-fit?
                     ; true.
                     (or (and multi? multi-fit?)
                         (<= (+ (or #_align-ind cur-ind) len 0 #_1) width)))))
        ...
        new-ind (cond
                  ...
                  ; onto a new line
                  (or comment? comment-inline?) (inc width)
                  ; If is multi-line, and we have multi-lines, and we don't
                  ; allow anything after a multi-line thing on the same line,
                  ; then force the next thing onto a new line
    ...
    [;
     ...
     ; changed
     param-map
     #_(assoc (dissoc param-map :excess-guided-newline-count :align-key :spaces)
         :cur-ind new-ind)
     ;
     ; previous-data
     ;
             ...
             (if fit?
               ...
               (if (not (zero? index))
                 ...
                 (concat-no-nil
                   ...
                   ; run
                   ; together!
                   [[(blanks actual-spaces) :none :whitespace 25]]
                   next-seq)
                 ...
                 (if (and previous-newline?
                   ...
                   (do (dbg-s options
                       ...
                       (concat-no-nil
                         [[(blanks
                             prepend-nl-ind
                             #_(if previous-newline? #_ind (+ indent ind) 1))
...
(defn guided-output-before-three-separate-approaches-good
  "Return information to be added to the output vector along
  with other information [param-map previous-data out].  Will do an
  fzprint* on zloc unless next-seq has data in it to use."
  ; [caller options next-seq zloc next-guide cur-index guide-seq
  ...
  [caller
   {:keys [width rightcnt],
    {:keys [wrap-after-multi? respect-nl?]} caller,
    :as options} next-seq zloc rightmost-zloc? next-guide #_cur-seq #_cur-zloc
   cur-index guide-seq element-index index multi-fit?
   {:keys [excess-guided-newline-count align-key last-cur-index rightcnt cur-ind
           ind indent spaces one-line-ind],
    :as param-map} mark-map
   [previous-newline? previous-guided-newline? unguided-newline-out?
    previous-comment? :as previous-data] out]
  ; We either have a zloc or we have a next-seq, but not both, so there
  ; are no decisions to make about which one to use.
  (let [guided-newline? (= next-guide :newline)
        next-seq (if guided-newline? [["\n" :none :newline]] next-seq)
        align-ind (when align-key (get mark-map align-key))
        align-spaces (when align-ind (max 0 (- align-ind cur-ind)))
        ; Figure out where we are on the line now, so we can call fzprint*
        ; If we have previous-newline? true, then we have a newline but
        ...
        ; somewhere on the line, and just have to put some spaces between
        ; where we are and where the next thing begin.
        regular-space (if (or previous-newline? (zero? element-index)) 0 1)
        additional-spaces (or align-spaces spaces 0)
        beyond-cur-ind (max additional-spaces regular-space)
        actual-spaces (if previous-newline?
                        (+ beyond-cur-ind cur-ind)
        ...
        hang-ind (+ beyond-cur-ind cur-ind)
        hang-cur-ind cur-ind
        try-hang? (and zloc
                       (not previous-newline?)
                       (not guided-newline?)
                       (< cur-ind width)
                       (< hang-ind width))
        hang-result (when try-hang? (fzprint* (in-hang options) hang-ind zloc))
        hang-lines (style-lines options hang-ind hang-result)
        hang-multi? (when hang-result (> (count hang-result) 1))
        hang-linecnt (when hang-lines (first hang-lines))
        ; This says that we either didn't try a hang or we did and the
        ; hang didn't fit.
        hang-fit? (and zloc
                       (and (not (empty? hang-result))
                            (if hang-multi? (<= hang-linecnt 1) true)))
        ; Test whatever we've got for fit
        next-seq (or hang-result next-seq)
        fail-fit? (or guided-newline?
                      ; this will cause a flow if the hang generated a newline,
                      ; which really isn't necessary, but 19 tests need this
                      ; to be here or this to be solved some other way
                      (= (nth (first next-seq) 2) :newline)
                      (= (nth (first next-seq) 2) :indent)
                      (= (nth (first next-seq) 2) :comment-inline)
                      (and (not (zero? element-index))
                           (= (nth (first next-seq) 2) :comment))
                      ; We tried a hang, and it didn't fit.
                      (and try-hang? (not hang-fit?)))
        ; If it didn't fit, we will flow it.  But if we flow it, we need
        ; to reset cur-ind.
        ;
        ; EXCEPT THIS ISN'T REALLY cur-ind.  It will be 0 once we do
        ; an \n.  But we need to ensure that we do prepend-nl.
        cur-ind (if fail-fit? (+ indent ind) #_0 cur-ind)
        _ (dbg-s options
                 :guide
                 "guided-output: hang-lines:" hang-lines
                 "hang-multi?" hang-multi?
        ...
        align-spaces (if fail-fit?
                       ...
                       align-spaces)
        spaces-after-indent (or spaces 0)
        flow-cur-ind cur-ind
        prepend-nl-ind
          (or align-ind
              (+ (if (zero? element-index) one-line-ind (+ indent ind))
                 spaces-after-indent))
        ; Flow if we have something to flow and we don't have a hang that
        ; fit (either because we didn't try the hang or because we did
        ; and the result didn't fit).
        flow-result (when (and zloc (not hang-fit?))
                      (fzprint* options prepend-nl-ind zloc))
        flow-lines
          (style-lines options prepend-nl-ind #_(+ indent ind) flow-result)
        _ (dbg-s options
                 :guide
        ...
        _ (dbg-s options
                 ...
                 "hang?" (not (empty? hang-result)))
        next-seq (or flow-result hang-result next-seq)
        ; This says that we don't fit if we did flow.
        fail-fit? (or fail-fit? flow-result)
        comment? (= (nth (first next-seq) 2) :comment)
        comment-inline? (= (nth (first next-seq) 2) :comment-inline)
        newline? (or (= (nth (first next-seq) 2) :newline) guided-newline?)
        ; Note that this isn't an :indent guide, this is an indent at
        ...
        ; line multi?, so in that case, len will be the length of the longest
        ; line by definition.
        len-base (cond flow-lines flow-cur-ind
                       hang-lines hang-cur-ind
                       :else cur-ind)
        len (if last-width (max 0 (- last-width len-base #_cur-ind #_ind)) 0)
        width (if (= index last-cur-index) (- width rightcnt) width)
        ; If it is multi? and (> linecnt 1), then it doesn't fit at present.
        ; If that changed, the fzprint-seq at the beginning would be wrong,
        ; because it says to do it at the "beginning".  Now, we could do
        ...
        ;
        ; If one line and fits, should fit.
        fit? (and #_(not newline?) ; includes guided-newline?
                  #_(not guided-newline?)
                  (not fail-fit?)
                  ; Is there an :indent element at the start of the first
                  ; thing we are doing for output?
                  #_(not indent?)
                  ; Inline comments don't fit because they need to be on a line
                  ; by themselves so that the inline-comment fixup routines can
                  ; do the right thing with them.
                  #_(not comment-inline?)
                  #_(or (zero? element-index) (not comment?))
                  ; Interesting that none of this matters.  I think it is all
                  ; handled elsewhere.
                  #_(or (zero? element-index)
                        ; hang-pairs
                        ; multi-fit? tells us whether a (possibly) multi-line
                        ; style-vec was generated at the "right" place, in which
                        ; case multi-fit? should be true, or just from
                        ; fzprint-seq
                        ; which would generate a multi-line thing at
                        ; (+ indent ind) and so it could never end up on the
                        ; same
                        ; line as something to the left of it because the 2nd
                        ; and subsequent lines wouldn't be in the right place.
                        (and
                          ...
                          (or (and multi? multi-fit?)
                              hang-fit?
                              #_(<= (+ (or #_align-ind cur-ind) len 0 #_1)
                                    width)))))
        ; Calculate new location on the line, which is the end of the thing
        ; we are outputing now.
        new-ind
...
; This doesn't even compile, and it is probably the wrong direction to
; go in anyway.
#_(defn guided-output-reorg
  "Return information to be added to the output vector along
  with other information [param-map previous-data out].  Will do an
  fzprint* on zloc unless next-seq has data in it to use."
  ; [caller options next-seq zloc next-guide cur-index guide-seq
  ...
  (let [guided-newline? (= next-guide :newline)
    ...
    (cond
      ;;
      ;; We got a zloc, so try a hang and then a flow
      zloc
        (let [hang-ind (+ beyond-cur-ind cur-ind)
              hang-cur-ind cur-ind
              try-hang? (and (not previous-newline?)
                             (not guided-newline?)
                             (< cur-ind width)
                             (< hang-ind width))
              hang-result (when try-hang?
                            (fzprint* (in-hang options) hang-ind zloc))
              hang-lines (style-lines options hang-ind hang-result)
              hang-multi? (when hang-result (> (count hang-result) 1))
              hang-linecnt (when hang-lines (first hang-lines))
              ; This says that we either didn't try a hang or we did and the
              ; hang didn't fit.
              hang-fit? (and (and (not (empty? hang-result))
                                  (if hang-multi? (<= hang-linecnt 1) true)))
              ; Test whatever we've got for fit
              next-seq hang-result
              [comment? comment-inline? newline? indent?] (seq-data next-seq)]
          (if (and hang-fit?
                   (not (or newline?
                            indent?
                            comment-inline?
                            (and (not (zero? element-index)) comment?))))
            ;;
            ;; The hang worked and we will use it
            ;;
            (let [_ (dbg-s options
                           :guide
                           "guided-output: hang-lines:" hang-lines
                  ...
                  _ (dbg-s options
                           ...
                           "cur-index:" cur-index
                           "element-index:" element-index)
                  [linecnt max-width lines] hang-lines
                  last-width (last lines)
                  ; This is the length of the last line,
                  ; not the length of the widest line,
                  ; though it might be.
                  len-base hang-cur-ind
                  len (if last-width
                        (max 0 (- last-width len-base #_cur-ind #_ind))
                        0)
                  width (if (= index last-cur-index) (- width rightcnt) width)
                  ...
                  new-ind (cond
                            ; Comments cause an overflow of the size,
                            ; forcing the next thing onto a new line
                            (or comment? comment-inline?) (inc width)
                            ; If is multi-line, and we have multi-lines, and we
                            ; don't
                            ; allow anything after a multi-line thing on the
                            ; same line,
                            ; then force the next thing onto a new line
                            (and multi? (> linecnt 1) (not wrap-after-multi?))
                              (inc width)
                            ; If it is multi-line, and it is more than one line,
                            ; and
                            ; we *do* allow things after the last line, then the
                            ; length
                            ; of the last line is the new cur-ind.
                            (and multi? (> linecnt 1)) last-width
                            :else (+ cur-ind len))
                  param-map
                    (dissoc param-map :excess-guided-newline-count :align-key)
                  param-map (assoc param-map :cur-ind new-ind)]
              (dbg-s
                options
                :guide "guided-output hang:"
                "\nindex:" index
                "\ncur-index:" cur-index
                "\nnewline?:" newline?
                "\nexcess-guided-newline-count:" excess-guided-newline-count
                ...
                "\nwidth:" width)
              ;;
              ;; Return [param-map [previous-data] out]
              ;;
              [;
               ; param-map
               ;
               ...
               (if (not (zero? index))
                 ...
                 next-seq)])
            ;;
            ;; The hang failed to fit or there are things that mean we
            ;; can't use it, so do a new line thing, which may be a flow
            ;; and may just reuse the hang-results
            ;;
            ;
            ; EXCEPT THIS ISN'T REALLY cur-ind.  It will be 0 once we do
            ; an \n.  But we need to ensure that we do prepend-nl.
            (let [cur-ind (if fail-fit? (+ indent ind) #_0 cur-ind)
                  align-spaces (when align-ind align-ind)
                  spaces-after-indent (or spaces 0)
                  flow-cur-ind cur-ind
                  prepend-nl-ind (or align-ind
                                     (+ (if (zero? element-index)
                                          one-line-ind
                                          (+ indent ind))
                                        spaces-after-indent))
                  ; Flow if we have something to flow and we don't have a hang
                  ; that
                  ; fit (either because we didn't try the hang or because we did
                  ; and the result didn't fit).
                  flow-result (when (not hang-fit?)
                                (fzprint* options prepend-nl-ind zloc))
                  flow-lines (style-lines options
                                          prepend-nl-ind
                                          #_(+ indent ind)
                                          flow-result)
                  _ (dbg-s options
                  ...
                  _ (dbg-s options
                           ...
                           "guided-output: flow?" (not (empty? flow-result))
                           "hang?" (not (empty? hang-result)))
                  ; If we did a flow, use it, else use the results of the hang
                  ; that either didn't fit or were not deemed correct for the
                  ; same
                  ; line
                  next-seq (or flow-result hang-result)
                  ; This says that we don't fit if we did flow.
                  ; fail-fit? (or fail-fit? flow-result)
                  [comment? comment-inline? newline? indent?] (seq-data
                                                                next-seq)
                  multi? (when next-seq (> (count next-seq) 1))
                  _ (dbg-s options
                           :guide
                  ...
                  [linecnt max-width lines]
                    (when next-seq
                      (or flow-lines
                          hang-lines
                          (style-lines options cur-ind next-seq)))
                  last-width (last lines)
                  ; This is the length of the last line, not the length of the
                  ; widest
                  ; line, though it might be.  That would be using max-width
                  ; instead.
                  ; But we only check the length to see if it fits if it is a
                  ; one
                  ; line multi?, so in that case, len will be the length of the
                  ; longest
                  ; line by definition.
                  len-base (cond flow-lines flow-cur-ind
                                 hang-lines hang-cur-ind
                  ...
                  ; we are outputing now.
                  new-ind (cond
                            ; Comments cause an overflow of the size, forcing
                            ; the next
                            ; thing
                            ; onto a new line
                            (or comment? comment-inline?) (inc width)
                            ...
                            ; of the last line is the new cur-ind.
                            (and multi? (> linecnt 1)) last-width
                            ; When this is (+ indent ind), that is part of what
                            ; makes
                            ; :spaces after a newline be "spaces beyond the
                            ; indent",
                            ; not "spaces instead of the indent".
                            ; Also, if (zero? guided-index), then we have put
                            ; some stuff
                            ; before the first :element in the guide.  The first
                            ; element
                            ; should be indented as if it were still on the same
                            ; line
                            ; as the l-str, and one-line-ind is that indent.
                            newline? (if (zero? element-index)
                                       one-line-ind
                                       (+ indent ind))
                            :else
                              ; the new last character is the length of the last
                              ; line
                              ; plus the ind and indent
                              (+ #_ind (+ indent ind) len))
                  param-map
      ...
      ;; next-seq
      next-seq
        (let [fail-fit? (or guided-newline?
                            ; this will cause a flow if the hang generated a
                            ; newline,
                            ; which really isn't necessary, but 19 tests need
                            ; this
                            ; to be here or this to be solved some other way
                            (= (nth (first next-seq) 2) :newline)
                            (= (nth (first next-seq) 2) :indent)
              ...
              _ (dbg-s options
                       :guide
                       "guided-output: next-seq: ind:" ind
                       "index:" index
                       "cur-index:" cur-index
                       "element-index:" element-index)
              [linecnt max-width lines]
                (when next-seq (style-lines options cur-ind next-seq))
              last-width (last lines)
              ; This is the length of the last line, not the length of the
              ; widest
              ; line, though it might be.  That would be using max-width
              ...
              ; we are outputing now.
              new-ind (cond
                        ; Comments cause an overflow of the size, forcing the
                        ; next
                        ; thing
                        ; onto a new line
                        (or comment? comment-inline?) (inc width)
                        ; If is multi-line, and we have multi-lines, and we
                        ; don't
                        ; allow anything after a multi-line thing on the same
                        ; line,
                        ; then force the next thing onto a new line
                        (and multi? (> linecnt 1) (not wrap-after-multi?))
                          (inc width)
                        ...
                        ; :spaces after a newline be "spaces beyond the indent",
                        ; not "spaces instead of the indent".
                        ; Also, if (zero? guided-index), then we have put some
                        ; stuff
                        ; before the first :element in the guide.  The first
                        ; element
                        ; should be indented as if it were still on the same
          ...
          [;
           ...
           (let [guided-output-out
                   (if fit?
                     ...
                     (if (not (zero? index))
                       (concat-no-nil
                         ; Ensure that despite alignment, we don't let two
                         ; things
                         ; run
                         ; together!
                         [[(blanks actual-spaces) :none :whitespace 25]]
                     ...
                     (if newline?
                       ...
                       (if (and previous-newline?
                         ...
                         (do (dbg-s options
                                    ...
                                    "guided-output: previous-newline? etc.")
                             (concat-no-nil
                               [[(blanks prepend-nl-ind
                                         #_(if previous-newline?
                                             #_ind
                                             (+ indent ind)
                                             1)) #_" " :none :whitespace 16]]
                               next-seq))
                         ...
                         (if indent?
                           ...
                           next-seq
                           (do
                             (dbg-s options :guide "guided-output: prepend-nl:")
                             (prepend-nl
                               options
                               ; This code is related to the code under
                               ; fit?
             ...
             (dbg-s options
                    :guide
                    (color-str "guided-output returned additional out:"
                               :bright-blue)
                    ((:dzprint options)
                      {:color? true}
                      guided-output-out
...
(defn seq-data
  "Take a next-seq and generate several booleans regarding what is there."
  [next-seq]
  (let [comment? (= (nth (first next-seq) 2) :comment)
        comment-inline? (= (nth (first next-seq) 2) :comment-inline)
        newline? (= (nth (first next-seq) 2) :newline)
        ; Note that this isn't an :indent guide, this is an indent at
        ; the start of the next-seq, which means a newline.
        indent? (= (nth (first next-seq) 2) :indent)]
    [comment? comment-inline? newline? indent?]))
...
(defn guided-output
  ...
  [caller
   {:keys [width rightcnt one-line?],
    {:keys [wrap-after-multi? respect-nl?]} caller,
    :as options} incoming-pairs zloc rightmost-zloc? next-guide #_cur-seq
   #_cur-zloc cur-index guide-seq element-index index multi-fit?
   {:keys [excess-guided-newline-count align-key last-cur-index rightcnt cur-ind
           ind indent spaces one-line-ind pair-seq],
    :as param-map} mark-map
   [previous-newline? previous-guided-newline? unguided-newline-out?
    previous-comment? :as previous-data] out]
  ; We either have a zloc or we have a incoming-pairs, but not both, so there
  ; are no decisions to make about which one to use.
  (let [guided-newline? (= next-guide :newline)
        ; incoming-pairs is [pair-ind pair-result]
	do-pairs? (= next-guide :pair-end)
	; TODO: REMOVE THIS
	multi-fit? do-pairs?
	; Forget incoming pairs
        ...
        ; Now incoming-seq is either the pair information or possibly a newline
        ; generated because we have a :newline in the guide.
        ;
        ; There are two fundemental approaches here -- this-line and next-line.
        ...
        try-this? (and (or zloc do-pairs?)
                       (not previous-newline?)
                       (not guided-newline?)
                       (< cur-ind width)
                       (< this-ind width))
        this-result (when try-this? 
		       (if do-pairs?
	                 (fzprint-pairs (in-hang options) this-ind pair-seq)
	                 (fzprint* (in-hang options) this-ind zloc)
		       ))
        ...
        try-next? (and (or zloc do-pairs?) (not this-fit?) (not output-newline?))
        ...
        next-result (when (and test-fit? try-next?)
		      (if do-pairs?
                        (fzprint-pairs (in-hang options) next-ind pair-seq)
        ...
        newline? (or (= (nth (first output-seq) 2) :newline) guided-newline?)
        ; Note that this isn't an :indent guide, this is an indent at
        ; the start of the output-seq, which means a newline.
        indent? (= (nth (first output-seq) 2) :indent)
...
(defn guide-pairs
  "Do the best we can to get pairs to actually fit.  This isn't a good-enough
  test, just to try to get it to actually fit. 
  Called with: [options param-map previous-data]
  Returns next-pair-seq which might be nil if we were already in-hang."
  [{:keys [in-hang? one-line?], :as options}
   {:keys [excess-guided-newline-count rightcnt cur-ind ind indent spaces
           pair-seq],
    :as param-map}
   [previous-newline? previous-guided-newline? unguided-newline-out?
    previous-comment? :as previous-data]]
  ; If we have excess-guided-newline-count, do we also have previous-newline?
  ; Should we set previous-newline? when index == 0?
  ; These are both things that should be investigated, but let's get
  ; the basics working first.
  (let [hang-ind (+ cur-ind (or spaces 1))
        pairs-hang (when-not previous-newline?
                     (fzprint-pairs (in-hang options) hang-ind pair-seq))
        pairs-hang (if (empty? pairs-hang) nil pairs-hang)
        flow-ind (+ indent ind)
        pairs-flow (when-not pairs-hang
                     (fzprint-pairs options flow-ind pair-seq))
        pairs-flow (when (not (empty? pairs-flow))
                     (if previous-newline?
                       pairs-flow
                       (concat [[(str "\n" (blanks (+ indent ind))) :none
                                 :indent 43]]
                               pairs-flow)))]
    #_(when (and pairs-flow
                 (not (or (= (nth (first pairs-flow) 2) :newline)
                          (= (nth (first pairs-flow) 2) :indent))))
        (println "pair which flowed and didn't have a newline or indent first!"
                 "pairs-flow:"
                 ((:dzprint options) {} (into [] pairs-flow))))
    ; Maybe (+ indent ind) and not ind for pairs-flow, but why doesn't
    ; it work at all?
    (dbg-s options
           :guide
           "guide-pairs: pairs-hang:"
             ((:dzprint options) {} (into [] pairs-hang))
           "\n  pairs-flow:" ((:dzprint options) {} (into [] pairs-flow))
	   "in-hang?" in-hang?
	   "one-line?" one-line?
           "ind:" ind
           "cur-ind:" cur-ind
           ...
           "spaces:" spaces
           "excess-guided-newline-count:" excess-guided-newline-count
           "pair-seq:" (map zstring pair-seq))
    ; If there is no output, don't return anything (i.e., don't output
    ; useless vectors).
    (when (or pairs-hang pairs-flow)
      (if pairs-hang [hang-ind pairs-hang] [flow-ind pairs-flow]))
    #_(or pairs-hang pairs-flow)))

(defn fzprint*-guide-alt
  "Call fzprint* for an element of a guide. Returns [multi-fit? local-seq]"
  [caller
   {:keys [width rightcnt one-line?],
    {:keys [wrap-after-multi? respect-nl? indent]} caller,
    :as options} {:keys [ind cur-ind indent], :as param-map} zloc rightmost?]
  [nil
   (fzprint* (if rightmost? options (not-rightmost options))
             (+ indent ind)
             zloc)])

(defn fzprint*-guide
  "Call fzprint* for an element of a guide.  If it is not multi-line then
  it tries to put it on the current line and if it doesn't fit, moves to the
  next line.  Multi-fit will be false then, but may not actually alter the
  behavior of guided-output.  Tries to put multi-line-things on the current 
  line, and does if at all possible.  If not possible, it puts them on the 
  next line. The way it changelines is by changing ensuring that multi-fit?
  is false for multi-line things. Returns [multi-fit? local-seq]"
  [caller
   {:keys [width rightcnt one-line?],
    {:keys [wrap-after-multi? respect-nl? indent]} caller,
    :as options} {:keys [ind cur-ind indent], :as param-map} zloc rightmost?]
  ; This has been hacked to always put a multi-line thing on the next line,
  ; never on the current line!
  (dbg-s options :guide "fzprint*-guide: cur-ind:" cur-ind)
  (let [hang-result (fzprint* (in-hang
                                (if rightmost? options (not-rightmost options)))
                              (inc cur-ind)
                              zloc)
        hang-multi?
          (and (not (empty? hang-result))
               (> (first (style-lines options (inc cur-ind) hang-result)) 1))
        flow-result (when (or (empty? hang-result) hang-multi?)
                      (fzprint* (if rightmost? options (not-rightmost options))
                                (+ indent ind)
                                zloc))
        multi-fit? false
        #_(not (empty? hang-result))]
    (dbg-s options
           :guide
           "fzprint*-guide: hang-lines:"
             (style-lines options (inc cur-ind) hang-result)
           "hang ind:" (inc cur-ind)
           "hang-result:" hang-result
           "flow-lines" (style-lines options (+ indent ind) flow-result)
           "flow ind:" (+ indent ind))
    [multi-fit?
     (if (and (not (empty? hang-result)) (not hang-multi?))
       hang-result
       flow-result) #_(or hang-result flow-result)]))

(defn fzprint*-guide-compare
  "Call fzprint* for an element of a guide.  Tries to put it on this and 
  the next line, and compares the result. Returns [multi-fit? local-seq]"
  [caller
   {:keys [width rightcnt one-line?],
    {:keys [wrap-after-multi? respect-nl? indent]} caller,
    :as options} {:keys [ind cur-ind indent], :as param-map} zloc rightmost?]
  (let [options (if rightmost? options (not-rightmost options))
        hang-result (fzprint* (in-hang options) (inc cur-ind) zloc)
        hang-lines (style-lines options (inc cur-ind) hang-result)
        hang-fit (fzfit options hang-lines)
        flow-result (fzprint* options (+ indent ind) zloc)
        flow-lines (style-lines options (+ indent ind) flow-result)
        flow-fit (fzfit options flow-lines)
        [multi-fit? result]
          (cond (and hang-fit flow-fit (<= (dec hang-fit) flow-fit))
                  [true hang-result]
                :else [false flow-result])]
    (dbg-s options
           :guide
           "fzprint*-guide-compare: hang-lines:"
           hang-lines
           "hang-fit:"
           hang-fit
           "flow-lines"
           ...
           multi-fit?
           #_"\nhang-result:"
           #_((:dzprint options) {} (into [] hang-result))
           #_"\nflow-result:"
           #_((:dzprint options) {} (into [] flow-result)))
    [multi-fit? result]))
...
(defn fzprint-guide
  ...
  (if (and one-line? (some #{:newline} guide))
    ...
    (let [rightcnt (fix-rightcnt rightcnt)
          last-cur-index (dec (count zloc-seq #_coll-print))]
      (when-not guide
        (throw (#?(:clj Exception.
                   :cljs js/Error.)
      ...
      (loop [cur-zloc zloc-seq
        ...
        (if (or (not (or guide-seq cur-zloc)) (nil? out))
          (do (dbg-s options
                     :guide
                     "fzprint-guide: out:"
          ...
          (if (> index 50)
            ...
            (let [_ (dbg-s options
                  ...
                  rightmost-zloc? (not (next cur-zloc))]
              (cond
                (and (:pair-seq param-map) (= (first guide-seq) :element))
                  ...
                  (let [[comments-or-newlines-cur-zloc remaining-cur-zloc]
                          (split-with zcomment-or-newline? cur-zloc)
                  ...
                  (if (:pair-seq param-map)
                    ...
                    (throw
                      (#?(:clj Exception.
                          :cljs js/Error.)
                       (str
                         ":pair-end without preceding :pair-begin!"))))
                (:pair-seq param-map)
                  ...
                  (throw
                    (#?(:clj Exception.
                     ...
                     (str "Unknown values: guide-seq:" (first guide-seq)
                          "\ncur-zloc:" (zstring (first cur-zloc)))))))))))))

(defn fzprint-guide-works-not-cleaned-up
  "Given a zloc-seq wrap the elements to the right margin 
  and be guided by the guide seq."
  [caller
   {:keys [width rightcnt one-line?],
    {:keys [wrap-after-multi? respect-nl? indent]} caller,
    :as options} ind cur-ind local-indent guide zloc-seq]
  #_(prn "fzprint-guide:" (map zstring zloc-seq))
  (dbg-s options
         :guide
         "fzprint-guide: entry:" (zstring (first zloc-seq))
         "caller:" caller
  ...
  (if (and one-line? (some #{:newline} guide))
    ...
    (do (dbg-s options
        ...
        nil)
    (let [rightcnt (fix-rightcnt rightcnt)
          #_#_coll-print (fzprint-seq options (+ local-indent ind) zloc-seq)
          last-cur-index (dec (count zloc-seq #_coll-print))]
      (when-not guide
        (throw (#?(:clj Exception.
                   :cljs js/Error.)
      ...
      (loop [#_#_cur-seq coll-print
        ...
        ; the last :element that finished cur-seq
        ;(if-not (or guide-seq cur-seq))
        (if (or (not (or guide-seq cur-zloc #_cur-seq)) (nil? out))
          (do (dbg-s options
                     :guide
                     "fzprint-guide: out:"
                     ((:dzprint options) {} (into [] (condense-depth 1 out))))
          ...
          (if (> index 50)
            ...
            (let [_ (dbg-s options
                  ...
                  ; First, look into what we have coming up in the sequence
                  ; we are formatting
                  #_#_comment? (= (nth (ffirst cur-seq) 2) :comment)
                  comment? (= (ztag (first cur-zloc)) :comment)
                  #_#_comment-inline?
                    (= (nth (ffirst cur-seq) 2) :comment-inline)
                  ; always used together
                  comment-inline? comment?
                  #_#_next-newline? (= (nth (ffirst cur-seq) 2) :newline)
                  next-newline? (= (ztag (first cur-zloc)) :newline)
                  rightmost-zloc? (not (next cur-zloc))]
              (cond
                (and (:pair-seq param-map) (= (first guide-seq) :element))
                  ...
                  ; First, split off any comments or newlines off of the
                  ; front of cur-seq and do the same for zloc-seq
                  (let [#_#_[comments-or-newlines-cur-seq remaining-cur-seq]
                          (split-with comment-or-newline? cur-seq)
                        ; Split zloc-seq the same way as cur-seq
                        #_#_[comments-or-newlines-cur-zloc remaining-cur-zloc]
                          (split-at (count comments-or-newlines-cur-seq)
                                    cur-zloc)
                    ...
                    (dbg-s options
                           ...
                           (color-str "fzprint-guide: === save a pair element"
                                      :bright-red))
                    (recur #_(next remaining-cur-seq)
                ...
                (= (first guide-seq) :pair-end)
                  ...
                  (if (:pair-seq param-map)
                    (let [next-pair-seq
                          ...
                          pair-seq (:pair-seq param-map)
                          #_#_param-map (dissoc param-map :pair-seq)]
                      ; We check the incoming :pair-seq here, because if
                      ; next-pair-seq is nil and :pair-seq was not empty,
                      ; that means that it didn't fit because we are in-hang?.
                      (cond
		  ...
		  ; the guide-seq.
                ...
                (= (first guide-seq) :element)
                  ...
                  (let [_ (dbg-s options
                                 ...
                                 (color-str "fzprint-guide: === :element"
                                            :bright-red))
                        [multi-fit? local-seq] (fzprint*-guide caller
                                                               options
                                                               param-map
                                                               (first cur-zloc)
                                                               rightmost-zloc?)
                        _ (dbg-s options
                                 :guide "fzprint-guide: element:"
                                 " multi-fit?" multi-fit?
                                 "\nlocal-seq:"
                                   (color-str
                                     ((:dzprint options) {} (into [] local-seq))
                                     :green))
                        [new-param-map new-previous-data new-out]
                          (if (not (empty? local-seq))
                            (guided-output
                              caller
                              options
                              nil
                              ...
                              previous-data
                              out)
                            ; we had a failure to fit
                            [param-map previous-data nil])]
                    (recur #_(next cur-seq)
                           (next cur-zloc)
                           (inc cur-index)
                           (next guide-seq)
                           ...
                           new-previous-data
                           new-out))
                #_#_(= (first guide-seq) :element-compare)
                  ; Do basic guided output, moving both cur-seq and guide-seq
                  (let [_ (dbg-s options
                                 :guide
                                 (color-str
                                   "fzprint-guide: === :element-compare"
                                   :bright-red))
                        [multi-fit? local-seq] (fzprint*-guide-compare
                                                 caller
                                                 options
                                                 param-map
                                                 (first cur-zloc)
                                                 rightmost-zloc?)
                        _ (dbg-s options
                                 :guide "fzprint-guide: element-compare:"
                                 " multi-fit?" multi-fit?
                                 "\nlocal-seq:"
                                   (color-str
                                     ((:dzprint options) {} (into [] local-seq))
          ...
          (if (> index 50)
            ...
            (let [_ (dbg-s options
              ...
              (cond
                ...
                ; All of the others are (and must be) processed above.
                ;
                ; Do we have any cur-seq to look at?
                #_(nil? cur-seq)
                (nil? cur-zloc)
                  ; No, nothing left in cur-seq.  This isn't a failure, because
                  ; we might be accumulating pairs or just have made too long
                  ; a guide.  Just keep going and eat up the guide-seq.
                  (do (dbg-s options
                             :guide
                             (color-str "fzprint-guide: === ran out of cur-seq"
                                        :bright-red))
                      (recur #_(next cur-seq)
                             (next cur-zloc)
                             (inc cur-index)
                ...
                (= (first guide-seq) :element-align)
                  ...
                  (let [align-key (first (next guide-seq))
                        ...
                        [multi-fit? local-seq] (fzprint*-guide caller
                                                               ...
                                                               rightmost-zloc?)
                        _ (dbg-s options
                                 :guide "fzprint-guide: element-align:"
                                 " multi-fit?" multi-fit?
                                 "\nlocal-seq:"
                                   (color-str
                                     ((:dzprint options) {} (into [] local-seq))
                ...
                (= (first guide-seq) :element)
                  ...
                  (let [_ (dbg-s options
                    ...
                    (recur #_(next cur-seq)
                           ...
                           new-previous-data
                           new-out))
                (= (first guide-seq) :element-compare)
                  ; Do basic guided output, moving both cur-seq and guide-seq
                  (let [_ (dbg-s options
                                 :guide
                                 (color-str
  ...
  (if (and binding? (= (:depth options) 1))
    ...
    (let [[respect-nl? respect-bl? indent-only?]
          ...
          #_(println "\nfirst in vector:" (zstring (zfirst zloc))
                     "\nfn-format local:" fn-format
                     "\nfzprint-vec* option-fn new options:" new-options)
          ...
          #_#_[guide options]
            (if option-fn
              [(:guide options) (if fn-format options (dissoc options :guide))]
              [nil options])
  ...
  (let [avail (- width indent)
    ...
    (cond
      ...
      (prefix-tags (ztag zloc))
        ; TODO: FIX THIS
        #_(do
            #_(println "ztag first zloc:" (ztag (zfirst zloc)))
            (cond
              (= (ztag (zfirst zloc)) :list)
                (fzprint-list* :list
                               (str (prefix-tags (ztag zloc)) "(")
                               ")"
                               (rightmost (prefix-options options (ztag zloc)))
                               indent
                               (zfirst zloc))
              (= (ztag (zfirst zloc)) :vector)
                (fzprint-vec* :vector
                              (str (prefix-tags (ztag zloc)) "[")
                              "]"
                              (rightmost (prefix-options options (ztag zloc)))
