  ...
  [caller
   {:keys [one-line? dbg? dbg-indent in-hang? do-in-hang? map-depth],
    {:keys [hang? dbg-local? dbg-cnt? indent indent-arg flow? key-color
            key-depth-color key-value-color respect-nl?]} ; i132
      caller,
            ...
            (if (and (zstring lloc)
              ...
              (let [; Perhaps someday we could figure out if we are already
                ...
                (if fit?
                  [:hang
                   (concat-no-nil arg-1
                                  [[(blanks hanging-spaces) :none :whitespace 3]]
                                  hanging)]
        ...
        (if-not remaining
          ...
          (let [[new-remaining pair-vec new-no-sort?]
            ...
            #_(println "partition-all-2-nc: count new-remaining:"
                       (count new-remaining))
            (dbg-pr options
                    "partition-all-2-nc: pair-vec: first:" (zstring
                                                               (first pair-vec))
...
(defn rstr-vec
  ...
  ([options ind zloc r-str r-type respect-nl?] ; i132
   (dbg-pr options "rstr-vec: respect-nl?" respect-nl?)
   (let [nl nil #_(when (and (zcomment? (zlast zloc)) (not respect-nl?))
...
(defn fzprint-flow-seq
  ...
  ([options ind zloc-seq force-nl? nl-first? respect-nl?]
   (dbg-pr options "fzprint-flow-seq: count zloc-seq:" (count zloc-seq) "nl-first?" nl-first? "respect-nl?" respect-nl? "zloc-seq:" (map zstring zloc-seq))
     ...
     (dbg-form
       ...
       (if (and (not force-nl?) (fzfit-one-line options one-line-lines))
         ...
         (if (not (empty? coll-print))
           (apply concat-no-nil (precede-w-nl options ind coll-print (not nl-first?) respect-nl?))
           :noseq)))))
  ([options ind zloc-seq] (fzprint-flow-seq options ind zloc-seq nil nil nil))
  ([options ind zloc-seq force-nl?]
   (fzprint-flow-seq options ind zloc-seq force-nl? nil nil))
  ([options ind zloc-seq force-nl? nl-first?]
   (fzprint-flow-seq options ind zloc-seq force-nl? nl-first? nil)))
...
(defn fzprint-get-zloc-seq
  ...
  (let [caller-options (caller options)
    ...
    (cond (:respect-nl? caller-options) (zmap-w-nl identity zloc)
     ...
     (dbg-pr options "fzprint-get-zloc-seq:" (map zstring zloc-seq)) ; i132
     zloc-seq
	  ))
      ...
      (let [[pre-next-zloc-seq next-zloc next-count] (gather-up-to-next-zloc
		...
		; output
	        coll-print (mapv vector coll-print)
                ...
                ; newlines
        ; i132       coll-print 
	; i132	  (if (:respect-nl? (caller options)) ; i132
	; i132	     coll-print
	; i132	     (map (partial add-newline-to-comment ind) coll-print))
...
(defn get-zloc-seq-right
  ...
  (if (>= next-count (count zloc-seq))
    ...
    (let [zloc-seq (nthnext zloc-seq (inc next-count))]
      #_(prn "get-zloc-seq-right: next-count:"  next-count "zloc-seq:" (map zstring zloc-seq))
...
(defn length-before
  ...
  (let [[up-size next-zloc] (left-or-up zloc)]
    (loop [ploc next-zloc
      ...
      (if-not ploc
        (do #_(prn "length-before: if-not ploc:" indent-before ) indent-before) ; i132
...
(defn next-actual
  ...
  (loop [nloc zloc]
	...
	#_(prn "nloc:" nloc "next-actual: next-nloc:" (zstring next-nloc) "next-tag:" next-tag)
        ...
        (let [this-seq (first cur-seq)
              new-seq (if (vector? (first this-seq))
			      ...
			      this-shift (if (and next-seq
			                          (not 
			                            (vector? (first next-seq)))
						  (= (nth next-seq 2) :indent))
			                   0
					   shift-ind)]
                          (if (= type :indent)
                            [(str s (blanks this-shift)) color type 42]
                            this-seq)))
              _ (dbg-pr options
         ...
         (let [this-seq (first cur-seq)]
           (when this-seq
             (let [multi? (> (count this-seq) 1)
                   ...
                   newline-after? nil #_comment?  ; i132
                           ...
                           (concat-no-nil
                             ...
                             [[(str "\n"
                                    (blanks (if l-str-indent?
                                              actual-ind
                                              actual-indent)
                                            #_actual-indent)) :none :indent 10]])
                           (concat-no-nil [[" " :none :whitespace 11]]
                                          this-seq
                                          [[(str "\n" (blanks actual-indent))
                                            :none :indent 11]]))
  ...
  ([caller l-str r-str options ind zloc fn-style arg-1-indent
   ...
   (let [flow-indent (:indent (caller options))
         ...
         already-hung? (when (and indent-only-style
                         ...
                         (hang-zloc? (zprint.zutil/down* zloc) #_(first zloc-seq)))  ; i132
    ...
    (cond
      ...
      (= len 1)
        ...
        (concat-no-nil l-str-vec
                       (fzprint-flow-seq roptions one-line-ind zloc-seq nil nil (:respect-nl? (caller options))) ; i132
                       r-str-vec)
	  ...
	  (dbg options "fzprint-list* :pair-fn respect-nl?" (:respect-nl? (caller options))) ; i132
          (concat-no-nil l-str-vec
                         pre-arg-1-style-vec
                         (fzprint* loptions (inc ind) arg-1-zloc)
                         (fzprint-hang (assoc-in options    ; i132
			                   [:pair :respect-nl?] 
					   (:respect-nl? (caller options)))
                                       :pair-fn
                                       arg-1-indent
        ...
        (let [[pre-arg-3-style-vec arg-3-zloc arg-3-count _ :as third-data]
          ...
          (concat-no-nil
            ...
            (cond (and doc-string? mixins?)
                    (concat-no-nil pre-arg-3-style-vec doc-string (remove-one-newline mixins))
                  doc-string? (concat-no-nil pre-arg-3-style-vec doc-string)
                  mixins? (remove-one-newline mixins) ; i132
                  :else :noseq)
                ...
                (let [local-indent (if (= fn-style :flow)
                  ...
                  (concat-no-nil ;[[(str "\n" (blanks local-indent)) :none
                    ...
                    (fzprint-flow-seq (noarg1 options fn-style)
                                      ...
                                      :force-nl
                                      :newline-first
				      (:respect-nl? (caller options)))))) ; i132
...
(defn wrap-zmap
  ...
  [caller
   {:keys [width rightcnt], {:keys [wrap-after-multi? respect-nl?]} caller, :as options} ind
   coll-print]
  #_(prn "wz:" coll-print)
  (let [last-index (dec (count coll-print))
    ...
    (loop [cur-seq coll-print
           ...
           out []]
      (if-not cur-seq
	(do (dbg-pr options "wrap-zmap: out:" out)
        out)
        (let [next-seq (first cur-seq)]
          (when next-seq
            (let [multi? (> (count (first cur-seq)) 1)
                  ...
                  comment? (if respect-nl? nil (= (nth (first this-seq) 2) :comment))
                  comment-inline? (if respect-nl? nil (= (nth (first this-seq) 2) :comment-inline))
                  width (if (= index last-index) (- width rightcnt) width)
                  ; need to check size, and if one line and fits, should fit
                  fit? (and (not newline?)
              ...
              (recur
		       ...
		       ; elements are.  Used to just be (blanks (dec new-ind))
		       ...
		       #_(if (first (next cur-seq))
		                            (dec new-ind)
					    new-ind)
					    ; Original no-indent version
				#_	    (dec new-ind)
  ...
  [caller l-str r-str
   {:keys [rightcnt in-code?],
    {:keys [wrap-coll? wrap? binding? option-fn-first option-fn respect-nl?
      ...
      caller,
    :as options} ind zloc]
  (dbg options "fzprint-vec* ind:" ind "indent:" indent "caller:" caller "respect-nl?:" respect-nl?)
...
(defn precede-w-nl
  ...
  ; respect-nl? currently not used i132
  [options ind coll not-first? respect-nl?]
  (dbg-pr options "precede-w-nl: (count coll)" (count coll) "not-first?" not-first? "respect-nl?" respect-nl?)
  (loop [coll coll
    ...
    (if (empty? coll)
      (let [result (persistent! out)
	    ...
	    previous-element-index (- (count result) 2)
	      ...
	      (nth (first (nth result previous-element-index)) 2))]
        (if nil #_(or (= previous-type :comment) (= previous-type :comment-inline))  ; i132
	  (butlast result) 
	  result))
      (let [[[s color what] :as element] (first coll)
            ...
            comment? nil #_(if respect-nl? nil
	    (or (= last-what :comment) (= last-what :comment-inline)))] ; i132
        (dbg-pr options "precede-w-nl: element:" element
			 ...
			 (conj! out element)))))
                 ...
                 (if added-nl?
                   ...
                   (if comment? 
                     (conj-it! out
                               element
                               [[(str "\n" (blanks indent)) :none :indent 25]])
                     (conj! out element))
                   ; We need both a newline and the element
                   (if comment? 
                     (conj-it! out
                               [[(str "\n" (blanks indent)) :none :indent 26]]
                               element
                               [[(str "\n" (blanks indent)) :none :indent 27]])
                     (conj-it! out
...
(defn repeat-element-nl-alt
  ...
  [n element]
  (prn "repeat-element-nl-alt: element:" element)
  (let [result (into [] (repeat n element))]
    (prn "repeat-element-nl-alt: output:" result)
    result))
...
(defn interpose-either-nl-hf-alt
  ...
  [sep-true sep-true-nl sep-nil sep-nil-nl
   {:keys [nl-separator? nl-separator-flow?], :as suboptions} ;nl-separator?
   pred-fn coll]
  #_(prn "ienf: sep-nil:" sep-nil "coll:" coll)
  (loop [coll coll
         out (transient [])
         previous-pred? nil
         add-nl? nil
    ...
    (if (empty? coll)
      (apply concat-no-nil (persistent! #_out
           ...
           out
           (conj-it! out
                     (repeat-element-nl newline-count (first sep-nil))
                     #_(repeat newline-count (first sep-nil)))) 
              ...
              (recur
                ...
                (if first?
                  (if (zero? addtl-nl-needed)
                    (conj! out style-vec)
                    (conj-it! out
                              (repeat-element-nl addtl-nl-needed (first sep-nil))
                              #_(repeat addtl-nl-needed (first sep-nil))
                              style-vec))
                  (if (zero? addtl-nl-needed)
                    (conj-it! out interpose-style-vec style-vec)
                    (conj-it! out
        ...
        (let [zstr (zstring zloc)
          ...
          (cond
			...
			(clojure.string/trimr zstr)
                        #_(clojure.string/replace-first zstr "\n" ""))
		    ; We don't need extra spaces on the right end of a comment
		    #_#_zcomment (clojure.string/trimr zcomment)
...
(defn find-element-from-end-alt
  "Find a the first element of this type working from the end of a 
  style-vec.  Return the index of the element."
  ...
  (loop [cvec style-vec
    ...
    (if-not cvec
      ...
      (let [[s c e :as element] (first cvec)
            ...
            new-element
              (cond
                (and (or (= e :indent) (= e :newline))  ; i132
		     (= ne :comment-inline))
