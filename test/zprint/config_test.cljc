    ...
    0 (:indent (:map options)))
        (with-redefs [zprint.config/configured-options
                      ...
                      zprint.config/explained-options
                        (atom zprint.config/default-zprint-options)
                      zprint.config/explained-sequence (atom 1)]
  (set-options! {:style :map-nl}) (get-options)))
    ...
    0 (:indent (:pair options)))
        (with-redefs [zprint.config/configured-options
                      ...
                      zprint.config/explained-options
                        (atom zprint.config/default-zprint-options)
                      zprint.config/explained-sequence (atom 1)]
  (set-options! {:style :pair-nl}) (get-options)))
...
(expect #?(:clj Exception :cljs js/Error)
        (with-redefs [zprint.config/configured-options
                        (atom zprint.config/default-zprint-options)
                      zprint.config/explained-options
                        (atom zprint.config/default-zprint-options)
...
#?(:clj (expect
          (more-of options 1 (get options :max-depth))
...
#?(:clj
     (expect
       (more-of options 2 (get options :max-depth) 22 (get options :max-length))
...
; Cached
#?(:clj (expect
          (more-of options 3 (get options :max-depth))
...
#?(:clj (expect
          (more-of options 44 (get options :max-depth))
...
#?(:clj (expect
          (more-of options 5 (get options :max-depth))
...
; Bad url, but cache
#?(:clj (expect
          (more-of options 6 (get options :max-depth))
