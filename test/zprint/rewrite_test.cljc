...
(java.util Date)))))")
  ...
  (expect "(ns a\n  (:require [a]\n            [b]\n            [c]))"
          (zprint-str "(ns a (:require [c] [b] [a]))"
                      {:parse-string? true, :style :sort-require}))

  (expect "(ns a (stuff :require [c] [b] [a]))"
          (zprint-str "(ns a (stuff :require [c] [b] [a]))"
                      {:parse-string? true, :style :sort-require}))

  (expect "(ns a\n  (#?(:clj :require\n      :cljs :require-macros)\n   [b]))"
          (zprint-str "(ns a (#?(:clj :require  :cljs :require-macros) [b]))"
                      {:parse-string? true, :style :sort-require}))
  ...
  (def i342 "(ns foo (:require [a.b.c :refer []]))\n")
